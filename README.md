...
The same Daily challenge is playable in the Web/Yandex build: the Game hub shows today's challenge above the catalog, each entry launches its deterministic puzzle directly (no difficulty choice), and results synchronize through Player-scoped local/cloud Daily storage. Profile now shows your Daily progress — completed challenges, current and best streak, and a few recent days — and a fully completed day can be shared spoiler-free through the browser (with a clipboard fallback). Daily 2048 resolves only at the real end of the game, unlike Catalog 2048 whose goal instantly clears the level. Payments, ads, Store, leaderboards, and achievements are not included.
