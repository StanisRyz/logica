...
Logica contains the native Android application and a playable Yandex Games Web Catalog for Balance, Crowns, Word, Sudoku, and 2048, plus a Daily challenge that is playable on both platforms. The deterministic core and canonical puzzle data are shared through `:puzzle-core`, their Compose Multiplatform presentation lives in `:shared-ui`, and neutral Store, Ads, Player, Cloud Save, lifecycle, and capability contracts live in `:platform-contracts`. Android remains the complete application; Web resumes and advances each game's saved Catalog level, shows Player-scoped gameplay statistics, and offers the same daily challenge, synchronized through Player-scoped local/cloud Daily storage.
...
Player-scoped local/cloud terminal gameplay statistics, and a separate compact Player-scoped Daily history payload,
...
The same Daily challenge is playable in the Web/Yandex build: the Game hub shows today's challenge above the catalog, each entry launches its deterministic puzzle directly (no difficulty choice), and results synchronize through Player-scoped local/cloud Daily storage. Profile now shows your Daily progress — completed challenges, current and best streak, a few recent days — and your Player-scoped wallet with gems and lives: solved Catalog puzzles earn gems by difficulty and failed ones cost a life. A new internal Store tab lets you spend those gems on hint packs and life restores, with per-Player inventory and purchase history; Sudoku hints now consume stored hints. A fully completed day can be shared spoiler-free through the browser (with a clipboard fallback). Daily 2048 resolves only at the real end of the game, unlike Catalog 2048 whose goal instantly clears the level. The Store also offers a clearly labelled rewarded placement — watch a short advertisement, receive +3 Sudoku hints. Short interstitial advertisements appear only after some Catalog victories when moving to the next level (never during gameplay or inside Daily), and the Yandex sticky banner shows around the Game hub, Profile, and Store while puzzles stay ad-free. Gem packs can be bought with real money directly through Yandex Payments; subscriptions, leaderboards, and achievements remain future stages. The application is currently Russian-only; it follows the language Yandex reports for safe defaults as localization expands. Progress is tied to your Yandex account automatically (no extra login) and the current Player state — catalog progress, statistics, Daily history, wallet, and inventory — now synchronizes across devices through one unified cloud-save path.
...
- `web-app/` — the browser/Yandex host with responsive 9:16 layout, browser data loading, lifecycle integration, automatic current-Player cloud synchronization through the unified Player save (catalog progression, statistics, Daily history, economy, and Store inventory), and lightweight controllers that run all five games from both the Catalog and the Daily challenge.
