...
- 2048 is the fifth production Catalog puzzle and uses `PuzzleType.GAME_2048`; it remains excluded from immutable Daily Policies V1–V4.
...
- `GAME_2048/CATALOG` is an independent Room v6 slot. `Game2048SessionCodecV1` serves both versions unchanged — it stores identity including the rules version, board, score, and `nextSpawnIndex` — and restore derives status from the persisted version while preserving the exact next spawn.
...
- Deferred: Daily V5 (Sudoku/2048 in Daily, 0/5..5/5 progress, streak qualification changes) and Yandex interstitial/fullscreen ads. Rewarded-life advertising is unchanged.
