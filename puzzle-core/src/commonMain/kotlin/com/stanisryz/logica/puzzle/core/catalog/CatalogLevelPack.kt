...
object CatalogLevelPackFormat {
    val MAGIC: ByteArray = "LOGLVP01".encodeToByteArray()
...
class BinaryCatalogLevelPack(
    ...
    private fun CatalogLevelPackInput.readFully(target: ByteArray): Boolean {
        var read = 0
    ...
    private fun CatalogLevelPackInput.skipFully(bytes: Long): Boolean {
        var remaining = bytes
        while (remaining > 0L) {
